...
def team_with_most_transactions(league):
    # Extract all recent league activities
    # Reduced size to fetch recent activities
    ...
    team_with_most_transactions = max(
        transaction_counts, key=lambda k: transaction_counts[k]["Claims"] + transaction_counts[k]["Trades"])
...
def team_with_most_injured_players(league):
    ...
    for team in league.teams:
        injured_players = [
            player.name for player in team.roster if player.injured]
    ...
    for box_score in box_scores:
        for player in box_score.home_lineup + box_score.away_lineup:
            if player.slot_position == 'BE' and player.points > benched_highest_points:
                ...
                benched_highest_player = (
                    player, box_score.home_team if player in box_score.home_lineup else box_score.away_team)
