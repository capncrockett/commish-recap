    ...
    for matchup in matchups:
        ...
        for team in teams:
            projected_diff = round(
                team.team_projected_points.total - team.team_points.total, 2)
...
def generate_weekly_recap(sc, week):
    ...
    highest_scorer, lowest_scorer, highest_scorer_bench, lowest_scorer_started, most_banged_up_team = find_extreme_scorers_and_banged_up_team(
        sc, team_ids, week)
...
def get_top_teams_string(sc):
    ...
    top_teams_string = ", ".join(
        [f"{team.name.decode('utf-8')} ({ordinal(team.team_standings.rank)} place - {team.team_points.total} points)" for team in top_3_teams])
