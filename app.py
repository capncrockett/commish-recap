...
def main():
    ...
    with st.sidebar:
        ...
        league_type = st.selectbox("Select League Type", [
                                   "Select", "ESPN", "Yahoo", "Sleeper"], key='league_type')
    ...
    if league_type != "Select":
        with st.sidebar.form(key='my_form'):
            if league_type == "ESPN":
                ...
                if cid is None or cse is None:
                    st.error(
                        "Client ID or Client Secret is not set. Please set the YAHOO_CLIENT_ID and YAHOO_CLIENT_SECRET environment variables.")
                    st.stop()

                # URL for st button with Client ID in query string
                # "oob"  # Out of band # "https://yahoo-ff-test.streamlit.app/" for dev version
                redirect_uri = "oob"
                ...
                st.write(
                    "1. Click the link below to authenticate with Yahoo and get the authorization code.")
                ...
                if st.session_state['auth_code'] and not st.session_state['access_token']:
                    ...
                    try:
                        r = requests.post(
                            'https://api.login.yahoo.com/oauth2/get_token', data=_data, auth=basic)
            ...
            st.text_input("Character Description", key='Character Description', placeholder="Dwight Schrute",
                          help="Describe a persona for the AI to adopt. E.g. 'Dwight Schrute' or 'A very drunk Captain Jack Sparrow'")
            st.slider("Trash Talk Level", 1, 10, key='Trash Talk Level', value=5,
                      help="Scale of 1 to 10, where 1 is friendly banter and 10 is more extreme trash talk")
        ...
        if submit_button:
            try:
                ...
                trash_talk_level = st.session_state.get(
                    'Trash Talk Level', 'Not provided')
                ...
                try:
                    ...
                    LOGGER.debug(
                        f"Generator object initialized: {gpt4_summary_stream}")
                    ...
                    with st.chat_message("Commish", avatar="🤖"):
                        ...
                        for chunk in gpt4_summary_stream:
                            ...
                            if chunk is not None:
                                ...
                                # Display partial message with a cursor-like symbol
                    ...
                    st.markdown(
                        "**Click the copy icon** 📋 below in top right corner to copy your summary and paste it wherever you see fit!")
                    st.code(full_response, language="")

                except Exception as e:
                    LOGGER.error(
                        f"An error occurred while streaming GPT response: {str(e)}")
